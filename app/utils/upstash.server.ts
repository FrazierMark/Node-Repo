...
export function createUpstashSessionStorage({ cookie }: any) {
  return createSessionStorage({
    ...
    async createData(data, expires) {
      ...
      await fetch(
        `${upstashRedisRestUrl}/set/${id}?EX=${expiresToSeconds(expires ?? new Date())}`,
        {
          method: "post",
    ...
    async readData(id) {
      ...
      try {
        ...
        return JSON.parse(data.result) as { data: any };
      } catch (error) {
        return null;
      }
    },
    async updateData(id, data, expires) {
      if (!expires) throw new Error("Expires date is required");
      await fetch(
